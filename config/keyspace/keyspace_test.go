...
// TestTheRegisteredKeySpaceIsTheOneThisPackageNames is the check the blank imports above exist for.
//
// Removing one of them compiles, links and passes every other test. The section it registered simply
// stops being in the key space, its keys resolve through the machinery that answered them before, and
// every report about them gets shorter rather than wrong. The other direction matters as much: a
// section nothing names is one nothing would notice leaving, which is how the set became a consequence
// of the import graph in the first place.
func TestTheRegisteredKeySpaceIsTheOneThisPackageNames(t *testing.T) {
	if drift := keyspace.Drift(); len(drift) > 0 {
		t.Errorf("the registered key space is not the one this package names:\n  %s\n\nA section named "+
			"here and not registered has its keys resolving through the machinery that answered them "+
			"before, with no diagnostic mentioning them, and the usual cause is a removed import above. "+
			"One registered and not named here needs adding, so that it failing to register later is a "+
			"failure rather than a shorter report", strings.Join(drift, "\n  "))
	}
}
...
func TestTheDeclaredSurfaceMatchesTheRecord(t *testing.T) {
	...
	configtest.CheckDeclaredSurface(t, "keyspace", registry.Surface())
}
